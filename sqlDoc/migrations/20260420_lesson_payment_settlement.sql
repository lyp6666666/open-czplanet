ALTER TABLE `tutor_appointment`
    ADD COLUMN `lesson_type` varchar(20) NOT NULL DEFAULT 'NORMAL' COMMENT '课节类型 TRIAL/NORMAL' AFTER `title`,
    ADD COLUMN `lesson_price_fen` bigint(20) UNSIGNED DEFAULT NULL COMMENT '单节标准课价（分）' AFTER `lesson_type`,
    ADD COLUMN `trial_price_percent` int(11) NOT NULL DEFAULT 50 COMMENT '试课收费比例，默认50表示半节课' AFTER `lesson_price_fen`,
    ADD COLUMN `payable_amount_fen` bigint(20) UNSIGNED DEFAULT NULL COMMENT '当前课节应付金额（分）' AFTER `trial_price_percent`;
...
ALTER TABLE `refund_request`
    ADD COLUMN `evidence_video_url` varchar(1024) DEFAULT NULL COMMENT '试课不通过微信聊天录屏URL' AFTER `evidence_images_json`,
    ADD COLUMN `evidence_video_duration_seconds` int(11) DEFAULT NULL COMMENT '录屏时长（秒，最长60秒）' AFTER `evidence_video_url`,
    ADD COLUMN `evidence_video_delete_status` varchar(32) DEFAULT NULL COMMENT '录屏删除状态 PENDING_DELETE/DELETED/KEEP' AFTER `evidence_video_duration_seconds`,
    ADD COLUMN `evidence_video_deleted_at` datetime(3) DEFAULT NULL COMMENT '录屏删除标记时间' AFTER `evidence_video_delete_status`;
