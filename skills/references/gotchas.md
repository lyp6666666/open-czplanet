...
- 当前信息费金额不是只在一个入口里算出来的。
  家教申请通过后的订单和合作提案订单都走 `videoCall-IM-service`，而且都应复用同一套频次阶梯规则：价格区间取均值、单值取原值、每周超过 5 次按 50%；改其中一条链路时要同步检查另一条。
- 当前用户端关于“为什么先收信息费”的长说明，已经刻意收口到聊天里的 `brokerage_required` 系统卡片入口。
  收银台页只保留简短保障与规则提示，不再承载完整说明弹层；后续如果继续补信任设计，优先沿这个单一入口演进，避免在多个页面复制出不同口径。
- 认证问题通常要同时看三层：
