...
- MySQL：`localhost:3306`
  ...
  - 约定（重要）：凡是涉及“库表结构变化”（新增表/字段/索引/约束/变更字段类型/长度/默认值等），必须同时更新：
    - 迁移脚本：`sqlDoc/migrations/*.sql`（用于增量升级）
    - 初始化脚本：`sqlDoc/huoyue.sql`（用于全量初始化）
    - 原则：保证新同学/新环境只导入 `huoyue.sql` 也能得到与当前代码匹配的最新表结构；避免“迁移脚本已改但初始化文件落后”导致环境不一致
...
- Nacos Console：`http://111.228.20.88:8848/nacos`
- Server Address（不带 `/nacos`）：`111.228.20.88:8848`
...
- 统一 Nacos 参数：
  - `spring.cloud.nacos.config.server-addr: ${NACOS_SERVER_ADDR:111.228.20.88:8848}`
  - `spring.cloud.nacos.config.group: DEFAULT_GROUP`
  - `spring.cloud.nacos.config.namespace: ${NACOS_CONFIG_NAMESPACE:${NACOS_NAMESPACE:beacf12a-2e06-43ae-80f1-6579081850a1}}`
...
- 业务服务：`tutor-appointment-service` / `videoCall-IM-service` / `payment-service`
- discovery server-addr 建议通过环境变量统一控制：
  - `spring.cloud.nacos.discovery.server-addr: ${NACOS_SERVER_ADDR:111.228.20.88:8848}`
- discovery namespace 统一通过环境变量覆盖（默认 DEV）：
  - `spring.cloud.nacos.discovery.namespace: ${NACOS_DISCOVERY_NAMESPACE:${NACOS_NAMESPACE:beacf12a-2e06-43ae-80f1-6579081850a1}}`

### 本地已验证的 Nacos 配置清单（最小可运行）

说明：当前本地已实现“核心配置迁移 Nacos”，服务能跑起来通常意味着以下 DataId 已在 Nacos 中配置完成（Group: `DEFAULT_GROUP`）。

- 公共（建议所有环境都存在）：
  - `ai-tutor-common-dev.yaml`：MySQL/Redis/JWT/MinIO/gateway.sign 等通用配置
  - `ai-tutor-home-dev.yaml`：首页推荐、Banner 等 Home 配置
  - `ai-tutor-sms-dev.yaml`：短信通道（Spug Push）配置
  - `ai-tutor-payment-dev.yaml`：支付通道密钥与回调地址（如使用）
- 服务专用（按需）：
...
- 由于 `spring.config.import` 使用 `optional:nacos:`，即使某个 DataId 缺失服务也可能继续启动，导致“看起来能跑但某些功能不可用”。建议用 `scripts/verify_nacos_effect.sh` 按日志核对 `Load config ... success` 是否齐全。
- 为避免开发环境共享 namespace 导致 discovery 串台，可使用双 namespace：
  - Config 用 `NACOS_CONFIG_NAMESPACE`
  - Discovery 用 `NACOS_DISCOVERY_NAMESPACE`
...
```bash
SPRING_PROFILES_ACTIVE=dev NACOS_SERVER_ADDR=111.228.20.88:8848 NACOS_NAMESPACE=beacf12a-2e06-43ae-80f1-6579081850a1
...
- 推荐使用脚本核对（按日志 + 简单 HTTP 探活）：
  - `bash scripts/verify_nacos_effect.sh`

5) 如需临时切换 Nacos 地址（仅本次启动生效）

```bash
sh ../mvnw -am spring-boot:run -Dspring-boot.run.arguments="--spring.cloud.nacos.discovery.server-addr=111.228.20.88:8848"
```
...
- 本地开发通常使用一键脚本拉起“网关 + 后端服务 + 两个前端 dev server”：
  - 启动：`sh scripts/dev_all_up.sh`
  - 停止：`sh scripts/dev_all_down.sh`
- 脚本参数（环境变量覆盖）：
  - `SPRING_PROFILES_ACTIVE`：默认 `dev`
  - `NACOS_SERVER_ADDR`：默认 `111.228.20.88:8848`
  - `NACOS_NAMESPACE`：默认 DEV 命名空间 `beacf12a-2e06-43ae-80f1-6579081850a1`
  - Nacos 双 namespace（避免配置/注册串台）：
    - `NACOS_CONFIG_NAMESPACE`：Nacos Config 使用（拉取 DataId 配置）
    - `NACOS_DISCOVERY_NAMESPACE`：Nacos Discovery 使用（服务注册发现）
    - 备注：两者默认都会回退到 `NACOS_NAMESPACE`
  - 端口：`GATEWAY_PORT/APPOINTMENT_PORT/IM_PORT/PAYMENT_PORT/ADMIN_PORT`（默认 18080~18084）
  - 前端端口：
    - `WEB_PORT`：用户端 `ai-tutor-web`（默认 5173）
    - `ADMIN_WEB_PORT`：管理端 `ai-tutor-admin-web`（默认 5174）
  - 日志与 PID：
    - 日志目录：`/.logs/*.log`
    - PID 目录：`/.pids/*.pid`
...
- 服务端通常使用打包后的 Jar 启动（而不是 `spring-boot:run`）：
  - 打包示例：`./mvnw -pl payment-service -am -DskipTests package`
  - 启动示例（按需替换模块、端口、profile、namespace）：

```bash
SPRING_PROFILES_ACTIVE=prod \
NACOS_CONFIG_NAMESPACE=<prod-config-namespace> \
NACOS_DISCOVERY_NAMESPACE=<prod-discovery-namespace> \
java -jar payment-service/target/*.jar \
  --server.port=18083
```

- 配置来源：各服务通过 `spring.config.import=optional:nacos:...` 从 Nacos 拉取；其中：
  - Config 侧读取的 namespace：`NACOS_CONFIG_NAMESPACE`
  - Discovery 侧注册的 namespace：`NACOS_DISCOVERY_NAMESPACE`
...
- 2026-03-25：将各服务 Nacos `server-addr` 在本地配置文件中写为环境变量可覆盖
...
- 2026-03-25：将 DEV 默认 namespace 固定为 `beacf12a-2e06-43ae-80f1-6579081850a1`（可通过环境变量覆盖）
