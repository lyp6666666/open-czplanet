...
case "$REMOTE_USE_TUNNEL" in
  ...
  0|false|no)
    ...
    ;;
  *)
    echo "[dev_remote_up] 不支持的 REMOTE_USE_TUNNEL=$REMOTE_USE_TUNNEL，可选值：1/0 true/false yes/no"
    exit 1
    ;;
esac

echo "[dev_remote_up] 远程开发模式"
echo "[dev_remote_up] remote=${REMOTE_USER}@${REMOTE_HOST}:${REMOTE_PORT}"
echo "[dev_remote_up] remotePath=$REMOTE_PATH"
echo "[dev_remote_up] remote MANAGE_INFRA=$REMOTE_MANAGE_INFRA"
echo "[dev_remote_up] remote NACOS_GRPC_CHECK=$REMOTE_NACOS_GRPC_CHECK"
...
echo "[dev_remote_up] remote ENABLE_AI_AGENT=$REMOTE_ENABLE_AI_AGENT"
echo "[dev_remote_up] remote AI_AGENT_HOST=$REMOTE_AI_AGENT_HOST"
...
case "$REMOTE_USE_TUNNEL" in
  1|true|yes)
    ...
    ;;
  0|false|no)
    echo "[dev_remote_up] 跳过本地 SSH 隧道，按远程地址直连"
    ;;
esac
...
ssh -p "$REMOTE_PORT" "${REMOTE_USER}@${REMOTE_HOST}" \
  "cd '$REMOTE_PATH' && MANAGE_INFRA='$REMOTE_MANAGE_INFRA' NACOS_GRPC_CHECK='$REMOTE_NACOS_GRPC_CHECK' NACOS_SERVER_ADDR='$REMOTE_NACOS_SERVER_ADDR' FRONTEND_HOST='$REMOTE_FRONTEND_HOST' ADMIN_WEB_BASE_PATH='$REMOTE_ADMIN_WEB_BASE_PATH' ENABLE_AI_AGENT='$REMOTE_ENABLE_AI_AGENT' AI_AGENT_HOST='$REMOTE_AI_AGENT_HOST' sh scripts/dev_all_up.sh"
