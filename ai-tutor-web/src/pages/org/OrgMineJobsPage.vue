...
async function loadMore() {
  ...
  error.value = null
  try {
    const page = await jobsApi.mineOrgDemands({ pageSize: 10, cursor: cursor.value })
...
function goDetail(id: number) {
  void router.push({ name: 'orgMineJobDetail', params: { id } })
}

function goEdit(id: number) {
  void router.push({ name: 'orgEditJob', params: { id } })
}
...
<template>
  <div class="wrap">
    <div class="head">
      <div class="title">机构需求</div>
      <button class="btn btn-primary" type="button" @click="router.push('/org/post')">发布新需求</button>
    </div>
    ...
    <div class="card list">
      <div v-if="list.length === 0 && !loading" class="empty">
        ...
        <div class="empty-desc">发布一条机构需求，让老师主动来开聊</div>
        <button class="btn btn-primary" type="button" @click="router.push('/org/post')">去发布</button>
      </div>

      <div v-else class="items">
        <div v-for="it in list" :key="it.id" class="item" :class="{ hl: highlightId != null && it.id === highlightId }">
          <div class="main">
            <div class="trow">
